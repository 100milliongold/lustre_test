...
subprocess.call('modprobe ost', shell=True)
subprocess.call('lsmod | egrep "ost"', shell=True)

for idx, val in enumerate(disk):
	subprocess.call('mkfs.lustre --fsname=mylustre --ost  --mgsnode='+host['mds']['host']+'@'+sysnetwork['network']+' --index='+str(host[ishost]['index'])+' --reformat '+val, shell=True)

for idx, val in enumerate(disk):
	subprocess.call('mkdir -p /lustre/ost'+str(idx+1), shell=True)

for idx, val in enumerate(disk):
	subprocess.call('mount -t lustre '+val+' /lustre/ost'+str(idx+1), shell=True)

for idx, val in enumerate(disk):
	subprocess.call('echo "'+val+'	/lustre/ost'+str(idx+1)+'	lustre defaults,_netdev 0 0" >> /etc/fstab', shell=True)
...
# subprocess.call('mkfs.lustre --fsname=mylustre --ost  --mgsnode=192.168.1.201@tcp1 --index=0 --reformat /dev/sdb', shell=True)
# subprocess.call('mkdir /mnt/ost1', shell=True)
# subprocess.call('mount -t lustre /dev/sdb /mnt/ost1', shell=True)
# subprocess.call('echo "/dev/sdb	/mnt/ost1	lustre defaults,_netdev 0 0" >> /etc/fstab', shell=True)
